...
export function PrototypeChrome({ active, paused, onPause }: { active: string; paused?: boolean; onPause?: () => void }) {
  ...
  return <>
    <div className="proto-review"><Link href="/prototypes">↖ {t("concepts")}</Link><div className="proto-directions">{["orbit", "studio", "signal"].map((name, i) => <Link key={name} href={`/prototypes/${name}`} aria-current={active === name ? "page" : undefined}>0{i + 1} <span>{name}</span></Link>)}</div><Button variant="ghost" className="proto-locale" onClick={() => setLocale(locale === "en" ? "cs" : "en")} aria-label={t("language")}>{locale === "en" ? "CS" : "EN"}</Button></div>
