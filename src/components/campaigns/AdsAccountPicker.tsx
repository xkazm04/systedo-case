    ...
    <div className="card p-5">
      ...
      {data?.configured === false ? (
        <div className="mt-3 flex items-start gap-2 rounded-card bg-navy-50 px-4 py-3 text-sm text-muted">
          ...
          Připojení živých Google Ads účtů vyžaduje developer token (viz <code>SETUP.md</code>).
          Zatím se používají ukázková data.
        </div>
      ) : (
        data?.configured && (
          <div className="mt-4">
            <p className="text-xs font-semibold uppercase tracking-wide text-muted">
              {connected.length ? "Přidat další účet" : "Připojit účet"}
            </p>
            {loading && !data.accounts?.length ? (
              <p className="mt-2 text-sm text-muted">Načítám dostupné účty…</p>
                ...
                {connectable.map((acc) => (
                  <li key={acc.customerId}>
                    ...
                    >
                      ...
                      <span className="shrink-0 text-xs font-semibold text-brand-accent">Připojit</span>
                    </button>
                  </li>
                ))}
              </ul>
              ...
              <p className="mt-2 text-sm text-muted">
                {connected.length
                  ? "Všechny dostupné účty jsou připojené."
