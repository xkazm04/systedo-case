...
/** The instant, deterministic report shown in the rail (no LLM call). */
export function reportFor(period: AnalysisPeriod): AnalysisResult {
  return demoAnalysis(buildSnapshot(period));
}

/** Four opening questions — the weakest paid channel is named so the first
 *  chip is specific to this period's data. */
export function reportChips(period: AnalysisPeriod, locale: SupportedLocale): string[] {
  const s = buildSnapshot(period);
