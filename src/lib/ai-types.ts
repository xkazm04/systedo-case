 ...
 *  telemetry entry into the CLIENT-visible meta so a degraded answer can say so.
 *    success  — parsed clean, first try (or after a plain provider retry)
 *    repaired — a re-prompt was needed to satisfy the tool's domain limits
 *    corrupt  — the parse is structurally broken (truncated / missing more than a
 *               third of the schema's required fields); the app still normalizes it
 *               so the screen isn't empty, but it may genuinely be incomplete. */
export type AiCallStatus = "success" | "repaired" | "corrupt";

/** Which rung of the JSON-extraction ladder produced the parse (see
 *  lib/llm/claude.ts `extractJson`). Recorded because the rung that fired is the
 *  LEADING indicator of producer/prompt drift: a flow that used to parse clean at
 *  `direct` and now routinely needs `balanced` has changed weeks before `balanced`
 *  starts missing too.
 *    direct   — the whole output was one JSON object
 *    fence    — it came wrapped in a ```json block
 *    envelope — reassembled from stream-json envelope lines
 *    balanced — recovered by a balanced-brace scan of arbitrary prose
 *  Absent for providers that parse natively (Gemini's responseSchema). */
export type AiExtractionRung = "direct" | "fence" | "envelope" | "balanced";
...
export interface AiMeta {
  ...
  demo: boolean;
  /** The exact prompt sent to the model. Carries every SERVER-resolved grounding
   *  (competitor lists, catalog rows, twin voice, lead/diagnosis data), so it is a
   *  DEBUG channel, not a client one: `/api/ai` strips it from the response and
   *  from the durable L2 cache unless `DEV_INSPECT=1` (see api/ai/dispatch.ts).
   *  Optional for exactly that reason — a consumer must degrade when it is absent. */
  prompt?: string;
  tookMs: number;
   ...
   *  client so the UI can flag a possibly-incomplete answer and support can
   *  correlate a complaint from the response alone. */
  status?: AiCallStatus;
  ...
  /** Estimated USD cost of the call. ABSENT — never 0 — when the call cannot be
   *  priced: the dev Claude CLI reports no usage at all (subscription), and a model
   *  with no rate row in lib/llm/cost.ts has no estimate. A zero here would claim
   *  the call was free; an absence says we cannot price it. */
  estCostUsd?: number;
  /** Which extraction-ladder rung produced the parse, for prompt-drift monitoring.
   *  Absent for natively-parsed providers. */
...
export interface AdRequest {
  ...
  patterns?: string[];
  /** server-derived brand grounding (what the project sells + how it talks), resolved
   *  SERVER-SIDE from the caller's project via resolveBrandContext — never trusted from
   *  the client. Injected into the USER prompt only — like `patterns`/`refine` — so the
   *  system prompt + schema stay byte-identical and the LLM-eval golden fingerprint
   *  holds. Absent (→ no brand lines) for a project with no catalogue / no project
   *  context, so the demo path's request shape + cache key are unchanged. */
  brand?: string;
