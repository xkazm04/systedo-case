...
| Finding out what previous runs got wrong before repeating it | [`docs/agent-lessons.md`](agent-lessons.md) | Each trap an agent has actually fallen into here, the shape of the correct call, and whether anything would stop you — `fenced`, `partial` or `unfenced`. Data in [`.github/agent-lessons.json`](../.github/agent-lessons.json) |
...
| Finding out which decision governs the file you already have open | [`.github/adr-ownership.json`](../.github/adr-ownership.json) | The reverse of the ADR table at the bottom of this page: `npm run adr:owner -- <path>` answers with a path instead of with a task, for the reader who is three files into a change and never framed it as a seam |
...
| A credential has leaked, or is due to be replaced | [`docs/runbooks/credential-rotation.md`](runbooks/credential-rotation.md) | The rotation procedure per credential class, which of the four shapes it is, and the `AUTH_SECRET` trap that orphans every stored tenant token. Data in [`.github/credential-rotation.json`](../.github/credential-rotation.json) |
...
| Saying which decision or rubric finding a commit acted on, or asking which commits acted on one | [`scripts/commit-decision.mjs`](../scripts/commit-decision.mjs) | The `Decision:` trailer: what may be cited (derived from the ADR filenames and the rubric's headings, never a list), why citing nothing is the common and correct case, and the reverse query — `npm run commit:check -- --range <range> --decision ADR-0001` |
| Working out whether the context you are editing has any test pointing at it | [`.github/context-test-floors.json`](../.github/context-test-floors.json) | The five contexts that may not fall to zero, why each earns a floor, and what a new context starts with. `npm run context:coverage` ranks the rest |
...
| Touching anything that holds a credential — a key, a connector token, the cron guard | [`docs/security/threat-model.md`](security/threat-model.md) | Where each credential enters, rests and leaves, which contexts may see it, and which fence stands on each edge — including which of the lint seams are security and which is architecture. Each flow carries a `TM-nn` id, and `npm run threat:flows` says which suite asserts it |
| Renaming an npm script, or running a command the guidance named | [`scripts/guidance-commands.mjs`](../scripts/guidance-commands.mjs) | Every `npm run …` the guidance quotes, resolved against `package.json` — `npm run guidance:commands`. Blocking, so the diff that renames a script is what goes red, not the run that trips over it |
