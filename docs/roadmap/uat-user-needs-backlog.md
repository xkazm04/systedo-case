...
> **✅ E2 shipped**: the marquee first-impression surface (`BrandLanding` + `Crossroad`) is now fully `T()`-routed — hero eyebrow/headline/subhead/CTAs/channels label, the proof band, the closing CTA, and the crossroad section headers all render Czech for a `cs` visitor (driver-verified: 0 residual English on `/`). Marek's "je to celé anglicky" first-impression scratch is closed. **The backlog's "~199 sites" was a stale pre-2026-06-20 estimate** — a full audit of the rebrand-era marketing + authed product + AI/social components found the app-wide UI-string/currency sweep already shipped (2026-06-20, see localization memory); the only residual leak was one hardcoded `aria-label="Local map pack"` in `MapPackClient` (now `t("mapAria")`). Currency already follows locale via `createFormatters`. E2 is complete; no large sweep remained.
