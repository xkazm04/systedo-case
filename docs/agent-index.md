...
spec that does not exist, or a spec whose `name:` stops matching its filename is
a red build. Same rung and same reasoning as the task index (ADR-0007 — it passes
today, so red is a regression).
...
below in the same commit. The test above is what makes that involuntary rather
than remembered.

## The agents

| Agent | Reach for it when | What it may assume | Returns |
|---|---|---|---|
